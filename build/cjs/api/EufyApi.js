...
class EufyApi {
    ...
    async getUserinfo() {
        return await this.requestClient({
            ...
            this.userInfo.gtoken = crypto_1.default.createHash('md5').update(res.data.user_center_id).digest('hex');
            return res.data;
        })
            .catch((error) => {
    ...
    async getDeviceList(device_sn) {
        const devices = await this.requestClient({
            ...
            if (data.devices) {
                ...
                if (device_sn?.length) {
                    return deviceArray.find((device) => device.device_sn === device_sn);
                }
            }
    ...
    async getDeviceProperties(deviceModel) {
        const base64 = [];
        const base64ToHex = [];
