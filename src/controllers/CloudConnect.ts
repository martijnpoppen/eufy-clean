...
export class CloudConnect extends Base {
    ...
    constructor(config: { login: string, password: string, deviceId: string, autoUpdate?: number }, openudid: string) {
        super();

        this.eufyLogin = new EufyLogin(config.login, config.password, openudid);
    ...
    async connect() {
        await this.eufyLogin.softLogin();
        await this.updateDevice(true);
    }

    async updateDevice(checkApiType = false) {
        const device = await this.eufyLogin.getCloudDevice(this.deviceId);
        // const device = {
        //     dps: {
        //         '150': '',
        //         '151': true,
    ...
    async getCleanSpeed(): Promise<CleanSpeed> {
        return this.robovacData.CLEAN_SPEED;
    }
        ...
        try {
            if (this.novelApi) {
                ...
                if (value?.warn?.length) {
                    const code = await getFlatData('./proto/cloud/error_code_list_t2320.proto', 'ErrorCodeList', value?.warn[0]);
                    return code
                }
            }
